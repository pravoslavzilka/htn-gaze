...
cal = {"active": False, "target": None, "text": "", "result": None, "i": 0, "n": 0}
...
def calibrate_worker(port):
    try:
        ...
        if not _read_status():
            ...
            return
        cal["active"], cal["result"] = True, None
        ...
        for i, ((x, y), col) in enumerate(zip(CAL_POINTS, CAL_COLORS)):
            cal["i"] = i
            cal["target"] = {"x": x, "y": y, "size": CAL_SIZE, "rgb": SC.RGB[col]}
            ...
            while time.time() < t_end:
                st = _read_status()
                time.sleep(0.05)
                if not st or st["t"] == last_t or st.get("eyes_closed") or st.get("gaze_raw") is None:
                    continue
                last_t = st["t"]
                fw, fh = st["frame_w"], st["frame_h"]
                objs = [o for o in st["seeing"] if o["color"] == col and o["area_pct"] >= 1.0]
                if not objs:
                    continue
                o = max(objs, key=lambda o: o["area_pct"])
                g_s.append(st["gaze_raw"])
                t_s.append([o["cx"] / fw, o["cy"] / fh])
            if len(g_s) >= 6:
                raws.append(np.median(g_s, axis=0).tolist())
                targets.append(np.median(t_s, axis=0).tolist())
            else:
                log.append(f"[cal] point {i + 1} ({col}): only {len(g_s)} usable samples, skipped")
        cal["target"] = None
        cal["text"] = "fitting..."
        if len(raws) < 6:
