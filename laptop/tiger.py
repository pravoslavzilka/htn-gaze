...
class TigerWriter:
    def __init__(self, dsn, flush_s=1.0, max_rows=50000, connect=None):
        ...
        self.buf = collections.deque(maxlen=max_rows)
        self.lock = threading.Lock()
        self.conn, self.next_try, self.backoff = None, 0.0, 1.0
        self.inserted = self.dropped = self.failures = 0
    ...
    def start(self):
        if not self.enabled:
            log.warning("TIGER_DSN not set: frames are not stored")
            return self
        self.thread.start()
        return self
    ...
    def flush(self):
        with self.lock:
            rows = list(self.buf)
        if not rows or time.monotonic() < self.next_try:
            return
        try:
            ...
            self.failures += 1
            log.error("Tiger insert failed (%s); %d rows buffered, retry in %.0fs", e, len(rows), self.backoff)
            try:
                if self.conn:
                    self.conn.close()
            except Exception:
